...
### if you have rasters, you'll need to first use the `raster_to_dataframe()` function
...
The function returns two vectors: the first vector gives the predicted values of the dependent variable for the invasive and susceptible landcovers before invasion. The second vector gives the predicted values of the dependent variable, assuming the invasive landcover as replaced all susceptible landcovers.

The function also returns two rasters: the first is the raster of predicted values before invasion, and the second is a raster of predicted values post-invasion.

NOTE: all landcovers that are not invasive or susceptible are given the original values of their dependent variable. This prevents issues when plotting the data, since we do not want to see a change in values for landcovers not being considered in the analysis. Doing this instead of giving them a value of `NA` helps to plot the rasters, where we usually want to color missing values differently than values of 0.
...
#plot
LandCoverPlot(pred_values$`Predicted values raster, change`, legend_title = 'ET difference', font_size = 20)
...
LandCoverPlot(pred_values$`Predicted values raster, change`, legend_title = 'ET difference', font_size = 20, low = 'blue', high = 'red')
...
# plot
LandCoverPlot(pred_values$`Predicted values raster, change`, value_type = 'priority', decimal_points = 2, legend_title = 'Change in ET', font_size = 20)
...
LandCoverPlot(pred_values$`Predicted values raster, change`, value_type = 'priority', decimal_points = 2, priority_colors = c('black', 'tan', 'orange', 'blue', 'gray', 'limegreen'),
...
LandCoverPlot(pred_values$`Predicted values raster, change`, value_type = 'priority', decimal_points = 2, legend_title = 'Change in ET', font_size = 20, priority_outlier_value = 1)
