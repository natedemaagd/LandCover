...
plot_PredictedVsActual <- ggplot(data = dat) +
  geom_histogram(aes(x = ET - ET_predicted, fill = as.character(landcover), color = as.character(landcover)), alpha = 0.5, bins = 40) +
...
# typically, we'll just use the `gls_spatial_predict` function to predict ET values for a single landcover. That's easy. Predict values of ET assuming landcover `1` invades everything:
...
ET_predicted_LC1_raster_LC1  <- gls_spatial_predict(data = dat, reg_results = regression_results, landcover = 1, return_raster = TRUE, x_coords = dat$x, y_coords = dat$y)
